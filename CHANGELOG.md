...
### 🔧 Fixed
- **יציבות בדיקת תמיכת atomic (timeout/hang)** — `wp_elementor_capabilities` ו-`wp_elementor_add_atomic` היו נתקעים עד ~90 שניות כשה-route `elementor-atomic-status` היה איטי או נרשם זה עתה: בקשת ה-GET עברה דרך מנגנון ה-retry הכללי (3 ניסיונות × timeout 30 שניות), וכל קריאת `add_atomic` הריצה מחדש את הבדיקה הכבדה (registry של Elementor). כעת הבדיקה עוברת דרך helper ייעודי `probeAtomicStatus` עם timeout קצר (8 שניות) ובלי retry אגרסיבי, כך שראוט חסר/איטי מתדרדר בחן ב-~8 שניות במקום לחסום את הכלי, ועם caching קצר-טווח (דקה לתשובה חיובית, 5 שניות לשלילית) שמונע בדיקות חוזרות. `wp_bootstrap_elementor_writer` מנקה כעת את ה-cache אחרי (re)install כך שתשובה שלילית ישנה לא "תשקר" אחרי force bootstrap, ואימות ה-route שלו חסום ל-12 שניות. בנוסף, שכבת בקשות ה-WP תומכת כעת ב-override של `timeoutMs`/`maxRetries` per-request.
